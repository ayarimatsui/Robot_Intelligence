...
class NeuralNetWork:
    ...
    def __init__(self,input_size=784,hidden_size_list=[512,1024,512],output_size=10,weight_init_std=0.1,load_params_dict=None):
        self.parameters={}
        ...
        self.parameters['W3']=weight_init_std*np.random.randn(hidden_size_list[1],hidden_size_list[2])
        self.parameters['b3']=np.zeros(hidden_size_list[2])
        self.parameters['W4']=weight_init_std*np.random.randn(hidden_size_list[2],output_size)
        self.parameters['b4']=np.zeros(output_size)
        ...
        self.layers['Sigmoid']=Sigmoid()  #変えてみる
        self.layers['Affine4']=Affine(self.parameters['W4'],self.parameters['b4'])
